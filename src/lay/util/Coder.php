...
class Coder {
    ...
    public function __construct($model = false, $client = false, $db = false) {
        $this->setModel($model);
        $this->setMongoClient($db);
        $this->setMongoDB($db);
    }
        ...
        if(empty($order)) {
            ...
            foreach($order as $field => $desc) {
                ...
                if($this->model) {
                    $columns = $this->model->columns();
                    if(array_search($field, $columns)) {
                        $this->order = array_merge($this->order, array($field=>$desc));
                    } else if(array_key_exists($field, $columns)) {
                        $this->order = array_merge($this->order, array($columns[$field]=>$desc));
                    } else {
    ...
    public static function array2Code($arr) {
        return $arr;
    }
}
?>
